function ret = savefigs(addedNameText,subfolder)
...
%         saveas(hfigs(m), [filename '.png']) %Standard PNG graphics file (best for web)
        ...
        mkdir(subfolder)
            ...
            print(gcf,'-depsc2','-loose',[pwd '/' subfolder '/Figure_' filename '_' addedNameText '.eps'])
            else 
            print(gcf,'-depsc2','-loose',[pwd '/' subfolder '/Figure_' filename '.eps'])
            end
        else if nargin > 0
        print(gcf,'-depsc2','-loose',['Figure_' filename '_' addedNameText '.eps']);
        else
         print(gcf,'-depsc2','-loose',['Figure_' filename '.eps']);
            end
        end
